 ...
 * 托管状态
 * 算子状态：
 *  联合列表状态：可以当成一个List来使用，状态管理、备份、恢复都，本质还是列表状态
 ...
 *
 * 区别：
 *   1. 列表状态在还原状态时，会将并行实例的状态组合到一起，然后按照轮询的方式重新分配到每个并行实例中
 *   2. 联合列表状态在还原状态时，会将并行实例的状态组合到一起，将组合后的状态全部发送给每个并行实例中
 *
 */
public class Flink03_UnionListStateOfOperatorState {
